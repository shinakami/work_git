...
print("excercise(c)")
def linear_model(x):
    Gaussian=np.random.normal(0,0.04, 40)
    for i in np.arange(0, len(Gaussian)):
        if(Gaussian[i]>=0):
            eps=Gaussian[i]
    y=np.sin(2*x*np.pi)+eps
    return y
...
ax[0].plot(x_new, func_y_regress, 's-', color="blue", label="traning model_degree=5", linewidth=5)
...
x_new=np.linspace(x[0:14].min(), x[0:14].max(), 100)
func_y_regress=func(x_new)
ax[0].plot(x_new, func_y_regress, 'o-', color="red", label="traning model_degree=10", linewidth=2.5)
...
func=interpolate.interp1d(x, y_regress_LOOCV_0, kind="cubic")
x_new=np.linspace(x.min(), x.max(), 100)
func_y_regress_LOOCV_0=func(x_new)
ax[0].plot(x_new, func_y_regress_LOOCV_0, color="blue", label="leave-one-out degree=5", linewidth=10)

func=interpolate.interp1d(x, y_regress_LOOCV_1, kind="cubic")
x_new=np.linspace(x.min(), x.max(), 100)
func_y_regress_LOOCV_1=func(x_new)
ax[0].plot(x_new, func_y_regress_LOOCV_1, color="red", label="leave-one-out degree=10", linewidth=7)

func=interpolate.interp1d(x, y_regress_LOOCV_2, kind="cubic")
x_new=np.linspace(x.min(), x.max(), 100)
func_y_regress_LOOCV_2=func(x_new)
ax[0].plot(x_new, func_y_regress_LOOCV_2, color="green", label="leave-one-out degree=14", linewidth=2.5)
